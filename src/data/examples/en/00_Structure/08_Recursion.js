 ...
 * go into an infinite loop. If a terminating condition is not 
 ...
 * Notice how the drawCircle() function calls itself 
...
function drawCircle(x, radius, level) {
  ...
  // a certain value, in this case, 1. If a terminating condition is not 
  // specified, a recursive function keeps calling itself forever.
  const tt = (126 * level) / 4.0;
  fill(tt);
  ellipse(x, height / 2, radius * 2, radius * 2);
  if (level > 1) {  
    // 'level' decreases by 1 at every step and thus makes the terminating condition
    // (being not greater than 1) attainable.
    level = level - 1;  
